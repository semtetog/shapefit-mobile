// Scripts inline extraídos de onboarding.html
...
// Prevenir scroll completamente
        document.addEventListener('DOMContentLoaded', function() {
            // Prevenir scroll via touch (exceto em inputs e modal)
            ...
            // Prevenir scroll via mouse wheel
            ...
            // Prevenir scroll via teclado (exceto em inputs)
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            document.querySelector('input[name="sleep_time_bed"]').value = "00:00";
            document.querySelector('input[name="sleep_time_wake"]').value = "00:00";

            // Data de nascimento = hoje por padrão (usar data local, não UTC)
            ...
            document.querySelector('input[name="dob"]').value = today;
            ...
            async function checkIfUserCompletedOnboarding() {
                try {
                    ...
                    const response = await fetch(`${window.BASE_APP_URL}/api/get_user_info.php`, {
                        method: 'GET',
                        headers: {
                        ...
                        if (weightInfo) {
                            ...
                            if (weightInput && weightMessage && daysRemaining && daysText) {
                                if (!weightInfo.can_edit) {
                                    ...
                                    fetch(`${window.BASE_APP_URL}/api/get_dashboard_data.php`, {
                                        method: 'GET',
                                        headers: {
            ...
            async function getWeightEditInfo() {
                try {
                    ...
                    const response = await fetch(`${window.BASE_APP_URL}/api/get_dashboard_data.php`, {
                        method: 'GET',
                        headers: {
                            'Authorization': `Bearer ${token}`
                        }
            ...
            actionBtn.addEventListener('click', async () => {
                ...
                if (currentStepIndex === steps.length - 1) {
                    ...
                    try {
                        const token = getAuthToken();
                        const response = await fetch(`${window.BASE_APP_URL}/api/process_onboarding.php`, {
                            method: 'POST',
                            headers: {
                ...
                // Garantir que não passa do último
                if (nextStepIndex >= steps.length) {
                    nextStepIndex = steps.length - 1;
                    // Se o último também estiver escondido, encontrar o último visível
                    while (nextStepIndex >= 0 && steps[nextStepIndex].style.display === 'none') {
                        nextStepIndex--;
                    }
                }
            ...
            if (exitBtn) {
                exitBtn.addEventListener('click', () => {
                    // Sempre redirecionar para dashboard quando clicar em sair
                    window.location.href = '../dashboard.html';
                });
            }
