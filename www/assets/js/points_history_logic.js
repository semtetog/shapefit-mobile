// points_history_logic.js - Lógica específica inline do points_history.html
...
(function() {
    ...
    // Carregar dados
    async function initPointsHistoryPage() {
    ...
    function renderPointsPage() {
        ...
        const nextLevelTextEl = document.getElementById('next-level-text');
        
        if (pointsValueEl) pointsValueEl.textContent = formatNumber(pointsData.user_points);
        if (levelTagEl) levelTagEl.textContent = pointsData.level.name;
        if (levelProgressBarEl) levelProgressBarEl.style.width = pointsData.level.progress_percentage + '%';
        
        if (pointsData.level.is_max_level) {
            if (nextLevelTextEl) nextLevelTextEl.innerHTML = '<strong>Você está no nível máximo!</strong>';
        } else {
            if (nextLevelTextEl) {
                nextLevelTextEl.innerHTML = 
        ...
        if (monthSelect) {
            ...
            if (pointsData.available_months && pointsData.available_months.length > 0) {
                ...
                monthSelect.addEventListener('change', function() {
                    if (window.router) {
                        window.router.navigate(`/points_history?month=${this.value}`);
                    } else {
    ...
    // Evento quando a view Points History entra
    window.addEventListener('spa:enter-points_history', function() {
        initPointsHistoryPage();
    });
    
    // Evento quando a view Points History sai (opcional, para cleanup)
    window.addEventListener('spa:leave-points_history', function() {
